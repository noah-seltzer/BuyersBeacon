...
const ReviewerAvatar: FC<ReviewerAvatarProps> = ({ userId }) => {
  ...
  return (
    <Link
      href={`/profile/${userId}`}
      className="block relative h-8 w-8 hover:opacity-80 transition-opacity"
    >
      {avatarUrl ? (
        <Image
          src={avatarUrl}
...
export const ReviewsList: FC<ReviewsListProps> = ({ userId, pageSize = 5 }) => {
  ...
  const handlePageChange = (newPage: number) => {
    ...
    document
      .getElementById("reviews-list-top")
  ...
  return (
    <div className="space-y-4 relative" id="reviews-list-top">
      ...
      {reviews.map((review) => (
        <Card
          key={review.reviewId}
          className="p-4 border-border/30 hover:border-primary/30 hover:shadow-sm transition-all duration-300"
        >
